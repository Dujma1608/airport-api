...
namespace FlightManagementBlazorServer.Services
{
    public class PassengerService
    {
        ...
        {
            return await _httpClient.GetFromJsonAsync<Passenger>($"{BaseApiUrl}/EditPassenger/{passengerId}");
        }
        ...
        {
            var request = new HttpRequestMessage(HttpMethod.Put, $"{BaseApiUrl}/checkPassenger");
            request.Content = new StringContent(JsonSerializer.Serialize(passenger),
                Encoding.UTF8, "application/json");
            await _httpClient.SendAsync(request);
        }
